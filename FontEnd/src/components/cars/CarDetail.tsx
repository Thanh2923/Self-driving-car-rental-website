...
const CarDetail = ({ product }: ProductPageProps) => {
    if(!product) {
        return <div className='align-center'>Không tìm thấy sản phẩm.</div>;
    }
    return (
        <>
...
export const getServerSideProps: GetServerSideProps = async ({ params }) => {
    try {
      const { id } = params!;
      const response = await axios.get(`http://localhost:8000/api/car/getCarById/${id}`);
      return {
        props: {
